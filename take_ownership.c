                    ...
                    size_t newKeyLength)
{
    ...
    //marshal old secretkey
    if (oldKeyLength != 0)
    {
        rval = str2Tpm2bAuth(oldOwnerSecretKey, oldKeyLength, &oldSecretKey);
        if (rval != 0)
        {
    ...
    //marshal new secretkey
    rval = str2Tpm2bAuth(newOwnerSecretKey, newKeyLength, &newSecretKey);
    if (rval != 0)
    {
        ERROR("There was an error creating the new TPM2B_AUTH");
        return -2;
